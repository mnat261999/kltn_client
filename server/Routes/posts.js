...
router.post("/api/posts/update_post", auth, (req, res) => {
  Tag.find({ posts: { $elemMatch: { $eq: req.body.postId } } }).exec(
    (err, tags) => {
      const oldTag = [];
      tags.forEach((item) => {
...
router.get("/api/posts/location/:name", auth, (req, res) => {
  Post.find({ locationName: req.params.name }).exec((err, posts) => {
    if (err) return res.status(400).send(err);
    res.send(posts);
  });
});
