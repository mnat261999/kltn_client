...
function isRestricted(restrictedFunctions, func) {
  ...
  console.log(
    restrictedFunctions.some(
      (item) => item.function == func && item.amountOfTime > today
    )
  );
  if (
    restrictedFunctions.some(
...
function SaveNotification(notification) {
  if (
    JSON.stringify(notification.sentTo) != JSON.stringify(notification.sentFrom)
  ) {
...
router.put("/api/users/updatePrivate", auth, (req, res) => {
  User.findByIdAndUpdate(
    req.user._id,
    { $set: { privateMode: req.body.privateMode } },
    { new: true },
    (err, result) => {
      if (err) {
  ...
  User.findByIdAndUpdate(
    ...
    (err, result) => {
      if (err) {
        ...
        return res.json({
          success: true,
          message: "Đã đổi thành công ảnh đại diện",
        });
      }
    }
  );
  ...
  User.find({
    ...
    .exec((err, users) => {
      ...
      filteredUsers = filteredUsers.filter(
        (item) => !item.blockedUsers.includes(req.user._id)
      );
      console.log(filteredUsers);
      Group.find({
        $and: [{ title: { $regex: matchRegex }, user: { $in: req.user._id } }],
      })
        .skip(skip)
        .limit(limit)
...
router.put("/api/users/update/:id", auth, jsonParser, (req, res) => {
  ...
  User.findOne({ userName: req.body.userName.trim() }, (err, user) => {
    ...
    if (isValidUserName) {
      ...
      ).then((result) => res.json({ success: true, message: "Thành công" }));
    } else {
...
router.put("/api/users/follow/:followId", auth, (req, res) => {
  if (!isRestricted(req.user.restrictedFunctions, "Follow")) {
    ...
    res.status(200).json({
      restricted: true,
      restrictedFunction: req.user.restrictedFunctions.find(
        (item) => item.function == "Follow"
      ),
    });
  }
});
...
router.put("/api/users/block/:id", auth, (req, res) => {
  User.findOneAndUpdate(
    { _id: req.user.id },
    {
      $pull: { followings: req.params.id, followers: req.params.id },
  ...
  User.findOne(
    ...
    (err, user) => {
      ...
      if (!user)
        return res.json({
          success: false,
          message: "Sorry, your token is invalid, please gennerate new one!",
        });
