...
router.get(
  "/api/statistics/newAccountThisMonth",
  auth,
  admin,
  async (req, res) => {
...
router.get(
  "/api/statistics/newPostThisMonth",
  auth,
  admin,
  async (req, res) => {
    let year = req.query.year ? req.query.year : new Date().getFullYear();
    ...
    console.log(firstDate, lastDate);
    const postsThisMonth = await Post.find({
      $and: [
        { createdAt: { $gt: new Date(firstDate) } },
        { createdAt: { $lt: new Date(lastDate) } },
      ],
...
router.get("/api/statistics/numOfAccount", auth, admin, async (req, res) => {
  ...
  console.log(firstDate, lastDate);
  const usersLastYear = await User.find({
    $and: [
      { createdAt: { $gt: new Date(firstDate) } },
      { createdAt: { $lt: new Date(lastDate) } },
    ],
...
router.get(
  "/api/statistics/unusedAccountSinceBeginOfThisYear",
  auth,
  admin,
  (req, res) => {
    ...
    User.find({ updatedAt: { $lt: new Date(startOfYear) } })
      .count()
      .then((cnt, err) => {
        if (err) res.status(400).json(err);
