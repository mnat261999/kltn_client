...
router.post("/api/reports/getAll", auth, admin, (req, res) => {
  ...
  let filter =
    req.body.filter[0] == "all" ? ["post", "comment", "user"] : req.body.filter;
...
router.put(
  "/api/reports/restrictUserFunction",
  auth,
  admin,
  async (req, res) => {
    ...
    const updated = await user.save((err, user) => {
      if (err)
        return res.json({ success: false, message: "Lỗi! Vui lòng thử lại" });
      Report.findByIdAndUpdate(
        req.body.reportId,
        {
          $set: { status: true },
...
router.put(
  "/api/reports/deleteRestrictFunction",
  auth,
  admin,
  async (req, res) => {
    const user = await User.findOne({ _id: req.body._id });
