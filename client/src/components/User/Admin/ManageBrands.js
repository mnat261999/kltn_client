...
class ManageBrands extends Component {
  ...
  submitForm = (event) => {
    ...
    if (formIsValid) {
      console.log(dataToSubmit);
      this.props
        .dispatch(addBrands(dataToSubmit, existingBrands))
        .then((response) => {
    ...
    return (
      <div className="admin_category_wrapper">
        ...
        <div className="admin_two_column">
          <div className="left">
            <div className="brand_container">{this.showCategpryItems()}</div>
          </div>
          <div className="right">
            <form onSubmit={(event) => this.submitForm(event)}>
              ...
              {this.state.formError ? (
                <div className="errorLabel">PLease check yoour data!</div>
              ) : (
                ""
              )}
            </form>
